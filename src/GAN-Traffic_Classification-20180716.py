    ...
    def run_train(self, train_loader):
        ...
        for epoch in range(num_epochs):
            for i, (b_x, b_y) in enumerate(train_loader):
                ...
                if ((epoch) % self.batch_size) == 0 and (i % self.n_critic) == 0:
                    print("Epoch: [%2d] [%4d/%4d] D_loss: %.8f real:%.8f/fake:%.8f, G_loss: %.8f" %
                          ((epoch), (i),
                           len(train_loader.sampler) // self.batch_size,
                           D_loss.data, D_real_loss.data, D_fake_loss.data, G_loss.data))
                    if i % 20 == 0:
...
def show_figures(D_loss, G_loss):
    ...
    plt.plot(D_loss, 'r', alpha=0.5, label='gan_distance(D_loss-G_loss)')
